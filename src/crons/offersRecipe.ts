...
export const setOffersRecipe = async () => {
  try {
    ...
    consola.info(`[OFFERS] Start create { offers } recipe for DB name - { ${process.env.DB_NAME} } DB port - { ${process.env.DB_PORT} }`);
    const offers: IOffer[] | undefined = await getOffers();
    if (!offers) {
      consola.error('[OFFERS] recipe offers created errors');
    ...
    consola.info(`[OFFERS] Identify Size of { Offers } Redis: { ${sizeOfOffersRedis} } DB { ${sizeOfOffersDB} } count: { ${offerFormat.length} }  for DB name - { ${process.env.DB_NAME} } `);

    if (sizeOfOffersDB === sizeOfOffersRedis) {
      consola.info(`[OFFERS] Size of { Offers } in Redis the same like in DB :${sizeOfOffersDB}, don't need create recipe  for DB name - { ${process.env.DB_NAME} } `);
      return;
    }
    consola.info(`[OFFERS] Size of { Offers } from Redis and DB is different, lets create the recipe, sizeOfOffersDB:${sizeOfOffersDB}, sizeOfOffersRedis:${sizeOfOffersRedis}  for DB name - { ${process.env.DB_NAME} } `);
    ...
    outputStream.on(
      ...
      async () => {
        ...
        consola.success(`[OFFERS] File Offers(count:${offerFormat?.length}) created path:${filePath}  for DB name - { ${process.env.DB_NAME} } `);
        setTimeout(uploadS3SetSize, 6000, sizeOfOffersDB);
      },
    );

    outputStream.on('error', (err: any) => {
      consola.error('[OFFERS] Offer qz file recipe creating got error:', err);
    ...
    consola.error('[OFFERS] create offers recipe Error:', e);
  }
};
